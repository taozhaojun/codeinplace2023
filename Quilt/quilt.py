...
def main():
    ...
    canvas_width = columns * PATCH_SIZE
    canvas_height = rows * PATCH_SIZE
    ...
    for row in range(rows):
        for col in range(columns):
            ...
            if (row + col) % 2 == 0:
                draw_square_patch(canvas, start_x, start_y)
            else:
                draw_circle_patch(canvas, start_x, start_y)
    '''
    canvas = Canvas(CANVAS_WIDTH, CANVAS_HEIGHT)
...
def draw_circle_patch(canvas, start_x, start_y):
    # TODO: your code here
    # draws a salmon frame at (start_x, start_y)
